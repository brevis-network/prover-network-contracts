...
### **Staking Flow**
1. **Validation:** Prover exists and (if delegator) state is Active; The minimal amount per stake is 1 full token (1e18 wei).
...
- Partial exits enforce the same 1-token minimum on both the withdrawn assets and the remaining assets.
